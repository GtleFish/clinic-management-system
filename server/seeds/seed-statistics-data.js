...
/**
 * Seed data cho báo cáo thống kê
 * Tạo dữ liệu sample cho LichHen, LichSuKham, ThanhToan
 */

const seedStatisticsData = async () => {
  try {
    ...
    // 2. Tạo dữ liệu LichHen (Appointments) - 20 appointments trong 3 tháng qua
    const today = new Date();
    const appointments = [];

    for (let i = 0; i < 20; i++) {
      const appointmentDate = new Date(today);
      appointmentDate.setDate(today.getDate() - Math.floor(Math.random() * 90)); // Random trong 90 ngày qua
    ...
    for (const appt of appointments) {
      const exists = await knex('LichHen').where({ idLichHen: appt.idLichHen }).first();
      if (!exists) {
    ...
    // 3. Tạo dữ liệu LichSuKham (Exam records) cho những appointments đã khám
    ...
    if (headDoctors.length > 0) {
      for (const appt of completedAppointments) {
        const examExists = await knex('LichSuKham').where({ idBenhNhan: appt.idBenhNhan }).first();
        if (!examExists) {
      ...
      console.log(`✓ Inserted exam records for completed appointments`);
    } else {
      console.log('⏭️  Skipped LichSuKham records (no head doctors found)');
    }

    // 4. Tạo dữ liệu ThanhToan (Payments/Deposits)
    const allAppointments = await knex('LichHen').select('idLichHen', 'idBenhNhan', 'ngayHen');
    const payments = [];
    ...
    for (const payment of payments) {
      const exists = await knex('ThanhToan').where({ idThanhToan: payment.idThanhToan }).first();
      if (!exists) {
