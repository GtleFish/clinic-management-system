...
function isDaDat(trangThai: string): boolean {
  return ['da_dat', 'Đã đặt lịch'].includes(trangThai);
}

function getMinutesLate(gioHen: string): number {
  const [h, m] = gioHen.slice(0, 5).split(':').map(Number);
  const apt = new Date();
...
export default function QuanLyVanHanh() {
  ...
  const stats = useMemo(() => ({
    ...
    checkedIn: lichHenList.filter((a) => a.trangThai === 'cho_kham').length,
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="text-left px-4 py-3 font-medium text-gray-600">Giờ hẹn</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="text-center py-10 text-gray-400">Không có lịch hẹn nào</td>
              </tr>
            ) : (
              filtered.map((lh) => {
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4 grid grid-cols-2 gap-2 text-sm mb-5">
              ...
              <div><span className="text-gray-500">Giờ hẹn:</span> {lateDialog.gioHen.slice(0, 5)}</div>
              <div>
